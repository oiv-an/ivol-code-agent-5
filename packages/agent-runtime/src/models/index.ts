...
/**
 * Options for fetching router models
 */
export interface FetchRouterModelsOptions {
	...
	timeoutMs?: number
	/** Extension bundle path (optional, for testing) */
	extensionBundlePath?: string
	/** Extension root path (optional, for testing) */
...
export async function fetchRouterModels(options: FetchRouterModelsOptions): Promise<RouterModels | null> {
	...
	try {
		...
		// Create extension service with optional custom paths
		const serviceOptions: Parameters<typeof createExtensionService>[0] = {
			workspace: workspace || process.cwd(),
		}
		if (options.extensionBundlePath) {
			serviceOptions.extensionBundlePath = options.extensionBundlePath
		}
		if (options.extensionRootPath) {
			serviceOptions.extensionRootPath = options.extensionRootPath
		}

		service = createExtensionService(serviceOptions)
 ...
 * This is a simplified API that wraps fetchRouterModels.
 *
...
export async function getAvailableModels(
	providerSettings: ProviderSettings,
	options?: Omit<FetchRouterModelsOptions, "providerSettings">,
