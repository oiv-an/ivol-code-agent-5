...
build_extension() {
  echo "🔨 Building the IVOL Code extension..."
  pnpm -w vsix -- --out ../bin/ivol-code-agent-5-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/ivol-code-agent-5-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if ! code --list-extensions 2>/dev/null | grep -q "ivol.ivol-code-agent-5"; then
  code --install-extension ivol.ivol-code-agent-5 &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing IVOL Code evals database... "
...
read -p "💻 Do you want to build a new version of the IVOL Code extension? [currently $current_version] (y/N): " build_extension
