...
describe("read_file tool output structure", () => {
	...
	describe("Error Handling Tests", () => {
		...
		it("should include error for RooIgnore error", async () => {
			...
			// Verify - native format for error
			expect(result).toBe(
				`File: ${testFilePath}\nError: Access to ${testFilePath} is blocked by the .kilocodeignore file settings. You must try to continue in the task without using this file, or ask the user to update the .kilocodeignore file.`,
			)
		})
...
// kilocode_change start: immutable task handoff routing must not read a different task's rotating root file.
describe("read_file verified context handoff snapshots", () => {
	const handoffPath = "CONTEXT_RESTART.md"
	const ownHeader = "<!-- IVOL_CODE_CONTEXT_RESTART_V1 handoff_id=11111111-1111-4111-8111-111111111111 -->"
	const ownContent = `${ownHeader}\n# Task\nResume this task only.\n# Remaining work\nVerify the final change.\n`
	let task: any
	let provider: any
	...
	beforeEach(() => {
		const mocks = createMockCline()
		task = mocks.mockCline
		provider = mocks.mockProvider
		task.cwd = "/workspace"
		task.getContextHandoffReadContent = vi.fn().mockResolvedValue(ownContent)
		provider.getState.mockResolvedValue({ maxReadFileLine: 1 })
		pushToolResult = vi.fn()
		fsPromises.stat.mockReset().mockResolvedValue({ isDirectory: () => false })
		fsPromises.readFile.mockReset().mockResolvedValue("FOREIGN TASK CONTENT")
		vi.mocked(path.resolve).mockImplementation((...parts) => parts.join("/"))
		vi.mocked(countFileLines).mockReset().mockResolvedValue(1)
		vi.mocked(isBinaryFile).mockReset().mockResolvedValue(false)
		vi.mocked(readLines).mockReset().mockResolvedValue("FOREIGN TASK CONTENT")
		vi.mocked(parseSourceCodeDefinitionsForFile).mockReset()
		mockReadFileWithTokenBudget.mockReset().mockResolvedValue({
	...
	async function execute(
		files = [{ path: handoffPath }] as { path: string; lineRanges?: { start: number; end: number }[] }[],
		protocol: "native" | "xml" = "native",
	): Promise<string> {
		task.taskToolProtocol = protocol
		await readFileTool.execute({ files }, task, {
			askApproval: task.ask,
			handleError: vi.fn(),
			pushToolResult,
			removeClosingTag: (_tag, content) => content ?? "",
			toolProtocol: protocol,
		})
		return pushToolResult.mock.calls.at(-1)?.[0] as string
	}

	it.each(["native", "xml"] as const)(
		"returns the full own snapshot in %s, even after the root file rotates",
		async (protocol) => {
			const result = await execute(undefined, protocol)
			expect(task.ask).toHaveBeenCalledOnce()
			expect(task.getContextHandoffReadContent).toHaveBeenCalledWith(handoffPath)
			expect(result).toContain(ownHeader)
			expect(result).toContain("5 | Verify the final change.")
			expect(result).not.toContain("FOREIGN TASK CONTENT")
			expect(result).not.toContain("Showing only")
			expect(fsPromises.stat).not.toHaveBeenCalled()
			expect(fsPromises.readFile).not.toHaveBeenCalled()
			expect(readLines).not.toHaveBeenCalled()
			expect(countFileLines).not.toHaveBeenCalled()
			expect(isBinaryFile).not.toHaveBeenCalled()
			expect(mockReadFileWithTokenBudget).not.toHaveBeenCalled()
			if (protocol === "native") {
				expect(result).toContain(`File: ${handoffPath}\nLines 1-5:`)
			} else {
				expect(result).toContain(`<files>\n<file><path>${handoffPath}</path>\n<content lines="1-5">`)
				expect(result).toContain("</content>\n</file>\n</files>")
			}
		},
	)

	it("reads the own snapshot when the visible root file was deleted", async () => {
		fsPromises.stat.mockRejectedValue(new Error("ENOENT: missing root file"))
		expect(await execute()).toContain("Verify the final change.")
		expect(fsPromises.stat).not.toHaveBeenCalled()
		expect(task.didToolFailInCurrentTurn).toBeUndefined()
	})

	it("does not apply definitions-only settings to a full mandatory continuation read", async () => {
		provider.getState.mockResolvedValue({ maxReadFileLine: 0 })
		expect(await execute()).toContain("Verify the final change.")
		expect(parseSourceCodeDefinitionsForFile).not.toHaveBeenCalled()
	})

	it.each(["native", "xml"] as const)(
		"preserves explicit line ranges in %s without reading the live root file",
		async (protocol) => {
			...
			expect(result).toContain("2 | # Task\n3 | Resume this task only.")
			expect(result).toContain("5 | Verify the final change.")
			expect(result).not.toContain(ownHeader)
			expect(result).not.toContain("4 | # Remaining work")
			expect(readLines).not.toHaveBeenCalled()
		},
	)

	it("rejects an out-of-bounds range without falling back to the root file", async () => {
		const result = await execute([{ path: handoffPath, lineRanges: [{ start: 200, end: 220 }] }])
		expect(result).toContain("Invalid line range for the verified continuation snapshot")
		expect(task.didToolFailInCurrentTurn).toBe(true)
		expect(fsPromises.stat).not.toHaveBeenCalled()
	})

	it("leaves unrelated paths on the normal file-reading path", async () => {
		task.getContextHandoffReadContent.mockResolvedValue(undefined)
		provider.getState.mockResolvedValue({ maxReadFileLine: -1 })
		const result = await execute([{ path: "src/app.ts" }])
		expect(result).toContain("ordinary file content")
		expect(task.getContextHandoffReadContent).toHaveBeenCalledWith("src/app.ts")
		expect(fsPromises.stat).toHaveBeenCalledWith("/workspace/src/app.ts")
		expect(mockReadFileWithTokenBudget).toHaveBeenCalledOnce()
	})

	it("does not fetch a snapshot when ignore permissions reject the file", async () => {
		task.rooIgnoreController.validateAccess.mockReturnValue(false)
		const result = await execute()
		expect(result).toContain("blocked by the .kilocodeignore")
		expect(task.getContextHandoffReadContent).not.toHaveBeenCalled()
		expect(task.ask).not.toHaveBeenCalled()
		expect(fsPromises.stat).not.toHaveBeenCalled()
	})

	it("does not fetch a snapshot when the user denies approval", async () => {
	...
	it("fetches the immutable snapshot only after approval", async () => {
		task.ask.mockImplementation(async () => {
			expect(task.getContextHandoffReadContent).not.toHaveBeenCalled()
			return { response: "yesButtonClicked" }
		})
		expect(await execute()).toContain(ownHeader)
	})

	it("reports snapshot verification failure without retrying an ordinary file read", async () => {
		task.getContextHandoffReadContent.mockRejectedValue(new Error("Verified continuation snapshot is unavailable"))
		const result = await execute()
		expect(result).toContain("Verified continuation snapshot is unavailable")
		expect(result).not.toContain("FOREIGN TASK CONTENT")
		expect(task.didToolFailInCurrentTurn).toBe(true)
		expect(fsPromises.stat).not.toHaveBeenCalled()
	})

	it("refuses an oversized snapshot before tokenizing it", async () => {
		task.getContextHandoffReadContent.mockResolvedValue("x".repeat(2 * 1024 * 1024 + 1))
		expect(await execute()).toContain("exceeds the 2 MiB safety limit")
		expect(task.api.countTokens).not.toHaveBeenCalled()
		expect(task.didToolFailInCurrentTurn).toBe(true)
	})

	it("refuses a full snapshot above the token guard without returning a partial read-proof header", async () => {
		task.api.countTokens.mockResolvedValue(64_001)
		const result = await execute()
		expect(result).toContain("not truncated or consumed")
		expect(result).not.toContain(ownHeader)
		expect(task.didToolFailInCurrentTurn).toBe(true)
	})

	it("respects the remaining context budget even for an otherwise small snapshot", async () => {
		task.getTokenUsage.mockReturnValue({ contextTokens: 199_990 })
		const result = await execute()
		expect(result).toContain("does not fit the available context budget")
		expect(result).not.toContain(ownHeader)
	})

	it("reserves a legitimate large output limit without silently reducing it", async () => {
		const model = task.api.getModel()
		task.api.getModel.mockReturnValue({ ...model, id: "gpt-5-test", info: { ...model.info, maxTokens: 128_000 } })
		task.getTokenUsage.mockReturnValue({ contextTokens: 70_000 })
		task.api.countTokens.mockResolvedValue(2_000)
		expect(await execute()).toContain("does not fit the available context budget")
	})

	it.each([-1, 200_000, Number.POSITIVE_INFINITY])(
		"handles an output-capacity sentinel of %s without disabling a small handoff read",
		async (maxTokens) => {
			const model = task.api.getModel()
			task.api.getModel.mockReturnValue({ ...model, id: "gpt-5-test", info: { ...model.info, maxTokens } })
			expect(await execute()).toContain(ownHeader)
		},
	)

	it("uses a conservative local estimate when token counting fails and hides the raw error", async () => {
		task.api.countTokens.mockRejectedValue(new Error("private diagnostic not intended for the model"))
		const result = await execute()
		expect(result).toContain(ownHeader)
	...
	it("does not treat token-counting failure as a zero-cost read", async () => {
		task.getContextHandoffReadContent.mockResolvedValue("ж".repeat(40_000))
		task.api.countTokens.mockRejectedValue(new Error("Tokenizer unavailable"))
		expect(await execute()).toContain("not truncated or consumed")
		expect(task.didToolFailInCurrentTurn).toBe(true)
	})

	it("rejects excessive repeated ranges without allocating a large response", async () => {
		const result = await execute([
			{ path: handoffPath, lineRanges: Array.from({ length: 101 }, () => ({ start: 1, end: 5 })) },
		])
		expect(result).toContain("Too many line ranges")
		expect(task.api.countTokens).not.toHaveBeenCalled()
	})
})
// kilocode_change end
