...
describe("MessageManager", () => {
	...
	describe("Condense handling", () => {
		// kilocode_change start: context restart handoff rewind lifecycle coverage
		it("deletes the owned pending restart file when rewind removes its summary", async () => {
		...
		it("re-arms a consumed handoff when rewind removes its first assistant continuation", async () => {
			...
			try {
				mockTask.cwd = workspace
				const condenseId = "summary-consumed"
				const record = await writeContextHandoffFile({
					workspacePath: workspace,
					taskId: "task-consumed",
					condenseId,
					modelId: "gpt-test",
					summary: "continuation to restore",
					trigger: "automatic",
				})
				const [attachedSummary] = attachContextHandoffToSummary(
					[
						{
							role: "assistant",
							content: [{ type: "text", text: "continuation to restore" }],
							isSummary: true,
							condenseId,
				...
				const consumedAt = record.createdAt + 20
				const summary = { ...attachedSummary, contextHandoffConsumedAt: consumedAt }
				const continuationTs = record.createdAt + 10
				await deleteContextHandoffFileIfOwned({ workspacePath: workspace, handoffId: record.handoffId })

				mockTask.clineMessages = [
					{ ts: record.createdAt - 100, say: "user", text: "First" },
					{
						ts: record.createdAt,
						say: "condense_context",
						contextCondense: { condenseId, summary: "Summary" },
					},
					{ ts: record.createdAt + 1, say: "user", text: "Continue" },
					{ ts: continuationTs, say: "assistant", text: "Continued" },
				]
				mockTask.apiConversationHistory = [
					{ ts: record.createdAt - 100, role: "user", content: [{ type: "text", text: "First" }] },
					summary,
					{ ts: record.createdAt + 1, role: "user", content: [{ type: "text", text: "Continue" }] },
					{
						ts: continuationTs,
						role: "assistant",
				...
				await manager.rewindToTimestamp(continuationTs)

				const rewoundHistory = mockTask.overwriteApiConversationHistory.mock.calls[0][0]
				const rearmedSummary = rewoundHistory.find(
					(message: any) => message.contextHandoffId === record.handoffId,
				)
				expect(rearmedSummary.contextHandoffConsumedAt).toBeUndefined()
		...
		it("re-arms a consumed handoff when rewind removes its successful tool result", async () => {
			const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "ivol-message-manager-handoff-"))
			try {
				mockTask.cwd = workspace
				const condenseId = "summary-tool-result"
				const record = await writeContextHandoffFile({
					workspacePath: workspace,
					taskId: "task-tool-result",
					condenseId,
					modelId: "gpt-test",
					summary: "tool continuation to restore",
					trigger: "automatic",
				})
				const [attachedSummary] = attachContextHandoffToSummary(
					[
						{
							role: "assistant",
							content: [{ type: "text", text: "tool continuation to restore" }],
							isSummary: true,
							condenseId,
							ts: record.createdAt - 1,
						},
				...
				const consumedAt = record.createdAt + 30
				const summary = { ...attachedSummary, contextHandoffConsumedAt: consumedAt }
				const toolCallTs = record.createdAt + 10
				const toolResultTs = record.createdAt + 20
				await deleteContextHandoffFileIfOwned({ workspacePath: workspace, handoffId: record.handoffId })
				...
				mockTask.clineMessages = [
					...
					{ ts: toolResultTs, say: "user", text: "Tool result" },
				]
				mockTask.apiConversationHistory = [
					{ ts: record.createdAt - 100, role: "user", content: [{ type: "text", text: "First" }] },
					summary,
				...
				await manager.rewindToTimestamp(toolResultTs)
		...
		it("re-arms a consumed XML handoff when rewind removes its tagged persisted read result", async () => {
			const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "ivol-message-manager-handoff-"))
			try {
				mockTask.cwd = workspace
				const condenseId = "summary-xml-read"
				const record = await writeContextHandoffFile({
					workspacePath: workspace,
					taskId: "task-xml-read",
					condenseId,
					modelId: "gpt-test",
					summary: "XML continuation to restore",
					trigger: "automatic",
				})
				const [attachedSummary] = attachContextHandoffToSummary(
					[
						{
							role: "assistant",
							content: [{ type: "text", text: "XML continuation to restore" }],
							isSummary: true,
							condenseId,
							ts: record.createdAt - 1,
						},
				...
				const consumedAt = record.createdAt + 30
				const summary = { ...attachedSummary, contextHandoffConsumedAt: consumedAt }
				const xmlCallTs = record.createdAt + 10
				const xmlResultTs = record.createdAt + 20
				await deleteContextHandoffFileIfOwned({ workspacePath: workspace, handoffId: record.handoffId })
				...
				mockTask.clineMessages = [
					...
					{ ts: xmlResultTs, say: "user", text: "XML read result" },
				]
				mockTask.apiConversationHistory = [
					{ ts: record.createdAt - 100, role: "user", content: [{ type: "text", text: "First" }] },
					summary,
					{
						ts: xmlCallTs,
						role: "assistant",
						content: "<read_file><path>.ivol-code/CONTEXT_RESTART.md</path></read_file>",
					},
					{
						ts: xmlResultTs,
						role: "user",
						content: [{ type: "text", text: "[read_file] Result: continuation" }],
						contextHandoffContinuationForId: record.handoffId,
					},
				]

				await manager.rewindToTimestamp(xmlResultTs)
